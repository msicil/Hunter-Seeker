...
const compiledCampaign = require('../ethereum/build/BountyHub.json');
...
beforeEach(async () => {
	...
	bounty = await new web3.eth.Contract(JSON.parse(compiledCampaign.interface), bountyAddress);
});
...
describe('Bounties', () => {
	...
	xit('allows a manager to make a payment request', async () => {
		await bounty.methods.createRequest('Buy batteries', '100', accounts[1]).send({
			from: accounts[0],
			gas: '1000000'
		});
